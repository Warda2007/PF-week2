   ...
   string name;
    ...
    if (currentUser == 1)
    {
        for (int i = 0; i < user1Count; i++)
        {
            if (user1Cars[i] == name)
            {
                for (int j = i; j < user1Count - 1; j++)
                {
                    user1Cars[j] = user1Cars[j + 1];
                }
        ...
        addToHistory("User1 returned " + name);
    }

    // Remove from User2 list
    else
    {
        for (int i = 0; i < user2Count; i++)
        {
            if (user2Cars[i] == name)
            {
                for (int j = i; j < user2Count - 1; j++)
                {
                    user2Cars[j] = user2Cars[j + 1];
                }
        ...
        addToHistory("User2 returned " + name);
    }
